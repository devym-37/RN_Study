...
import { fitbox, processTransform2d, Skia, vec } from "@shopify/react-native-skia";

export const fitRect = (src: SkRect, dst: SkRect) => processTransform2d(fitbox("contain", src, dst));
...
export class PathGeometry {
    ...
    getPointAtLength(length: number) {
        const contour = this.contours.find(({ from, to }) => length >= from && length <= to);
        if (!contour) {
