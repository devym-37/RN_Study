...
const PaletteItem: FC<PaletteProp> = ({ colors, index, gestureDegree, activeColor, onColorPress }) => {
        ...
        <Animated.View style={[styles.paletteContainer, styles.paletteSize, viewStyle]}>
            {colors.map((color, index) => (
                <Pressable
                    ...
                    style={[
                        styles.colorItem,
                        index === 0 && styles.colorTop,
                        { backgroundColor: color, marginBottom: index < colors.length - 1 ? 4 : 0 },
                    ]}
...
const SpreadCards = () => {
    ...
    const calculateDegree = (e: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
        "worklet";
        console.log("e.y", e.y, "e.x", e.x);
        let degree = Math.atan2(PALETTE_HEIGHT - e.y, e.x - PALETTE_WIDTH / 2) * (180 / Math.PI);
                    ...
                    <View style={[styles.paletteSize, { margin: 40 }]}>
                        {COLOR_PALETTE.map((colors, index) => {
                            return (
                                <PaletteItem
                                    key={index.toString()}
                                    {...{ colors, index, activeColor, gestureDegree }}
