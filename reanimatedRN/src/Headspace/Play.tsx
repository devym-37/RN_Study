...
import { SkiaValue, SkPath, useComputedValue, Vector } from "@shopify/react-native-skia";
...
const Flubber2SkiaInterpolator = (from: SkPath, to: SkPath) => {
    const interpolator = interpolate(from.toSVGString(), to.toSVGString(), { maxSegmentLength: 10 });
    const d = 1e-3;
...
export const Play = ({ progress, c, r }: PlayProps) => {
    const left = useComputedValue(() => leftPath(progress.current), [progress]);
    const right = useComputedValue(() => rightPath(progress.current), [progress]);

    const sr = r * 0.45;
    return (
        <Group color='white' transform={fitbox("contain", bounds, rect(c.x - sr, c.y - sr, sr * 2, sr * 2))}>
            <Path path={left} />
            <Path path={right} />
        </Group>
    );
